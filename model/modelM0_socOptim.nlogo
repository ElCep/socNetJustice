extensions [
  nw         ; extension pour les réseaux
  gini.jar   ; extension de romain pour le gini des parcelles
  ;morphology ; extension de Juste pour calculer le moran des arbres
]
...
to small-world-ring
  generate [ -> nw:generate-watts-strogatz turtles undirected-edges num-nodes neighborhood-size rewire-prob ]
  ask turtles [
    set capital 0
  ]
end
...
to gini-on-capital
  ...
  let _giniActual calcul-gini
    ifelse _giniActual > gini-expected [
    ;; Si le Gini de la distribution du capital est superieur au gini attendu
    ;; il faut prendre un des plus fortuner et reduire sa fortune
    ;; let _pourestTurtle one-of turtles with-min[captial]
    ask one-of turtles with-max [capital][
      set capital capital / 2
    ]

  ][
    ;; Si le Gini de la distribution du capital est inferieur au gini attendu
    ;; il faut prendre un des plus fortuner et augmenter sa fortune
    ask one-of turtles with-max [capital][
      set capital capital * 2
    ]
  ]
...
to-report calcul-gini
  ...
  repeat num-nodes[
    ...
    set index (index + 1)
    set gini-index-reserve gini-index-reserve + (index / num-nodes) - (wealth-sum-so-far / total-wealth)
  ]
   set gini-index-reserve (gini-index-reserve / num-nodes) / 0.5
